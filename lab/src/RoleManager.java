...
public class RoleManager implements Repository<Role> {
    ...
    public boolean remove(Role role) {
        ...
        if (!assignmentManager.findAssignmentsByRole(role).isEmpty()) {
            throw new IllegalStateException("Не удается удалить роль '" + role.getName() + "' потому что он назначен пользователям");
        }
    ...
    @Override
    public void clear() {
        if (!assignmentManager.getAllAssignments().isEmpty()) {
            throw new IllegalStateException("Невозможно очистить роли, пока существуют назначения");
        }
        rolesById.clear();
        rolesByName.clear();
    }
    ...
    public boolean removeByName(String name) {
        ...
        if (!assignmentManager.findAssignmentsByRole(role).isEmpty()) {
            throw new IllegalStateException("Не удается удалить роль '" + name + "' потому что он назначен пользователям");
        }
